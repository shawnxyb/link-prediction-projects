...
def evaluate_ranking(
    ...
    """
    Compute per-user Precision@K, Recall@K, Hits@K, NDCG@K.
    ...
    for u in range(num_nodes):
        ...
        effective_k = min(k, scores.numel())
        topk = torch.topk(scores, k=effective_k).indices.detach().cpu().numpy().tolist()
        ...
        hits_list.append(hit_rate)
        ndcgs.append(ndcg)

    return {
        f"Precision@{k}": float(np.mean(precisions)) if precisions else 0.0,
        f"Recall@{k}": float(np.mean(recalls)) if recalls else 0.0,
        f"Hits@{k}": float(np.mean(hits_list)) if hits_list else 0.0,
