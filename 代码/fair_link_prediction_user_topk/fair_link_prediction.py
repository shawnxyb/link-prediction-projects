...
# -*- coding: utf-8 -*-
"""
Fair comparison for link prediction on SNAP ego-Facebook.
...
def download_and_load_graph(data_dir: str) -> nx.Graph:
    os.makedirs(data_dir, exist_ok=True)
    path = os.path.join(data_dir, "facebook_combined.txt.gz")
...
def build_user_eval_pairs(
    ...
    for u in sorted(positives_by_user):
        ...
        # n=4039, so building this list per evaluated user is acceptable.
        available_neg = [
            x for x in range(n)
...
def main():
    ...
    parser.add_argument("--out-dir", type=str, default="outputs_facebook_lp")
    ...
    G = download_and_load_graph(args.data_dir)
    n = G.number_of_nodes()
    full_edge_set = edge_set_from_graph(G)
    print(f"Loaded ego-Facebook: nodes={n}, edges={G.number_of_edges()}")
